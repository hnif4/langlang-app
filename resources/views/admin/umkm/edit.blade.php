...
@section('content')
    ...
    <div class="container mx-auto py-6">
        <form action="{{ route('admin.umkm.update', $item->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                <label for="gambar" class="block text-sm font-medium text-gray-700">Gambar Lama</label>
                <input type="file" name="gambar" id="gambar"
                       class="mt-1 block w-full" accept="image/*">
                @if($item->gambar)
                    <img src="{{ asset('storage/' . $item->gambar) }}" class="mt-2 w-32 rounded" alt="gambar">
                @endif
            </div>
